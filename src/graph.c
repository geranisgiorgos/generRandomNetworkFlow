...
void set_parms(long parms[],char *arguments[]){
    ...
    if(COMMAND_LINE==1) {
        parms[0]=NODES=atoi(arguments[2]); // nodes
        parms[1]=SOURCES=atoi(arguments[4]); // sources
        parms[2]=SINKS=atoi(arguments[5]); // sinks
        parms[3]=DENSITY=atoi(arguments[3]); // density
        parms[4]=MINCOST=atoi(arguments[8]); // mincost
        parms[5]=MAXCOST=atoi(arguments[9]); // maxcost
        parms[6]=SUPPLY=atoi(arguments[10]); // total supply
        parms[7]=TSOURCES=atoi(arguments[6]); // tsources
        parms[8]=TSINKS=atoi(arguments[7]); // tsinks
        parms[9]=HICOST=atoi(arguments[11]); // hicost
    } else{
...
void print_X(){
     ...
     for(i=1;i<NODES1;i++){
       if(Orientation[i]==0)
          fprintf(OUT,"X(%d,%ld)=%ld ",i+1,Tree[i],X[i]);
       else
          fprintf(OUT,"X(%ld,%d)=%ld ",Tree[i],i+1,X[i]);
...
void print_s(){
     ...
     fprintf(OUT,"\ns values:\n");
     for(i=0;i<DENSITY1;i++){
         fprintf(OUT,"s%ld.%ld=%0.1f ",FROM[i],TO[i],s[i]);
         }
     fprintf(OUT,"\n--------------------------------\n");
}
...
int read_from_file(char *filename){
    ...
    while (fgetc(infile)!='\n');
    for (i=0;i<DENSITY;i++){
        fscanf(infile,"%d%d%d%d%d",&x1,&x2,&x3,&x4,&x5);
//fscanf(infile,"%d%d%d%d",&x1,&x2,&x3,&x4);
        FROM[i]=x1; TO[i]=x2;C[i]=x3;
        }
    for(i=0;i<NODES;i++){
        fscanf(infile,"%d%d",&x1,&x2);
//        fscanf(infile,"%d",&x2);
        B[x1-1]=x2;
        B[i]=x2;
        }
...
void store_graph(char *store_file){
    ...
    for(i=0;i<DENSITY1;i++){
        fprintf(storef,"%ld %ld %ld 100000 0\n",FROM[i],TO[i],C[i]);
        }
    for(i=0;i<NODES1;i++){
        fprintf(storef,"%ld %ld\n",i+1,B[i]);
        }
...
void print_to_giden(){
    ...
    for(i=0;i<NODES1;i++){
        fprintf(outfile,"node:id=%ld,x=%ld,y=%ld\n",i+1,(i%4)*STEP+STEP,(i/4)*STEP+STEP);
    }
    for(i=0;i<DENSITY1;i++){
         fprintf(outfile,"edge:id=%ld,e1=%ld,e2=%ld,lp=1,spline=false\n",i+1,FROM[i],TO[i]);
    }
    fprintf(outfile,"node data labels:1,supply,i\n");
    for(i=0;i<NODES1;i++){
        fprintf(outfile,"node data values:%ld,%ld\n",i+1,B[i]);
    }
    fprintf(outfile,"edge data labels:2,cost,i,capacity,i\n");
    for(i=0;i<DENSITY1;i++){
         fprintf(outfile,"edge data values:%ld,%ld,%d\n",i+1,C[i],10000);
    }
}
...
void print_D(){
     ...
     printf("\nD vector:\n");
     for(i=0;i<DENSITY1;i++)
          printf("D(%ld,%ld)=%ld ",FROM[i],TO[i],D[i]);
     }
